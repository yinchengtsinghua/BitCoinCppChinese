...
public:
    ...
    {
        /*o locked_chain=m_wallet.chain（）.lock（true/*尝试_lock*/）；
        如果（！）洛克-德链）
            返回错误；
        }
        试试“锁定”（m wallet.cs wallet，locked wallet）；
        如果（！）锁好的钱包）
            返回错误；
        }
        auto mi=m_wallet.mapwallet.find（txid）；
    ...
    wallettx getwallettxdetails（const uint256&txid，
        WallettxStatus和Tx_状态，
        墙订单窗体和订单窗体，
        布尔和inou mempool，
        int&num_块）覆盖
    {
        自动锁定的_chain=m_wallet.chain（）.lock（）；
        锁（m欷wallet.cs欷wallet）；
        auto mi=m_wallet.mapwallet.find（txid）；
        如果（MI）！=m_wallet.mapwallet.end（））
            num_blocks=：：chainActive.height（）；
            在ou mempool=mi->second.in mempool（）中；
    ...
    walletbalances getbalances（）重写
    {
        钱包余额结果；
        result.balance=m_wallet.getbalance（）；
        result.unconfirmed_balance=m_wallet.getUnconfirmedBalance（）；
        result.undyname_balance=m_wallet.getUndynableBalance（）；
        result.have_watch_only=m_wallet.have watch only（）；
        if（result.have_watch_
            result.watch_only_balance=m_wallet.getbalance（仅限ismine_watch_）；
            result.unconfirmed_watch_only_balance=m_wallet.getUnconfirmedWatchOnlyBalance（）；
            result.undynamic_watch_only_balance=m_wallet.getundynamicwatchonlybalance（）；
        }
        返回结果；
    }
    bool trygetbalances（walletbalances&balances，int&num_块）覆盖
    {
        auto-locked_chain=m_wallet.chain（）.lock（真/*试锁*/);

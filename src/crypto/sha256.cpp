...
//一些随机输入数据用于测试
static const unsigned char data[641] = "-" //有意不结盟
...
//对上面的i*64第一个输入字节进行哈希处理所需的输出状态（不包括sha256填充）。
...
//完整双sha256（包括填充）下每个单独的8 64字节消息的预期输出。

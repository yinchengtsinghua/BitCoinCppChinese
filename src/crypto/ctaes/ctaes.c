...
/*持续时间、未优化、简洁、简单的C、AES实现
 *基于：
 *Emilia Kasper和Peter Schwabe，更快、及时地抵御攻击的AES-GCM
 *网址：http://www.iacr.org/archive/ches2009/57470001/57470001.pdf
 *但使用8个16位整数表示单个AES状态，而不是8个128位
 *表示8个aes状态的整数。
 **/
...
/*slice variable slice_i包含16个状态变量的第i位，顺序如下：
 ＊0 1 2 2
 ＊4 5 6 6
 ＊8 9 10 10
 ＊12 13 14 14
...
/*基于门逻辑的S盒实现来自：
 *Joan Boyar和Rene Peralta，用于AES S-Box的深度16电路。
 *网址：https://eprint.iacr.org/2011/332.pdf
**/
...
static void MixColumns(AES_state* s, int inv) {
    /*mixcolumns转换将状态列的字节视为
     *gf（2^8）上三次多项式的系数，并将其乘以
     *通过固定多项式a（x）=03 x^3+01 x^2+01 x+02，模
     *x^ 4＋{ 01 }。
     *
     *在逆变换中，我们乘以a（x）的逆矩阵，
     *A ^-1（x）=0b x ^3+0d x ^2+09 x+0e。这等于
     *a（x）*（04 x^2+05），因此我们可以重用前向转换的代码
     （在OpenSSL的bsaes-x86_64.pl中找到，归因于Jussi Kivilinna）
     *
     *在位片表示法中，每列乘以x。
     *mod x^4+1只是一个右旋转。
     **/
    ...
    /*两个方向的共享是A（x）的乘法，可以
     *重写为（x^3+x^2+x）+02*（x^3+01）。
     *
     *首先计算s到s？变量，（x^3+01）*s到s？Y01
     *变量和（x^3+x^2+x）*s到s？123个变量。
     **/
...
/*将s中的单元格乘以x，作为多项式乘以gf（2）mod x^8+x^4+x^3+x+1*/
...
/*将密码密钥展开到密钥计划中。
 *
 *状态必须是指向nRounds+1大小数组的指针。
 *键必须是指向4*nkeywords字节的指针。
 *
 *aes128使用nkeywords=4，nrounds=10
 *aes192使用nkeywords=6，nrounds=12
 *aes256使用nkeywords=8，nrounds=14
 **/

...
//第一个证书是签名证书，其余的是不受信任的证书链
//到有效的根颁发机构。OpenSSL需要单独使用它们。
...
//有效证书；检查签名：
payments::PaymentRequest rcopy(paymentRequest); //拷贝
        rcopy.set_signature(std::string(""));
std::string data_to_verify;                     //除了签名以外的一切
...
//用于从certs获取人类可打印字符串的openssl api是巴洛克风格的。
...
//TODO:检测EV证书并设置Merchant=business name而不是unfriendly nid_commonname？
    }

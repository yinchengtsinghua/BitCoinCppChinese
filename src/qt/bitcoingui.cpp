...
//对于具有分段进度条的样式，替代进度条的样式表，
//因为它们使文本不可读（解决问题1071）
//请参阅https://doc.qt.io/qt-5/gallery.html
...
//安装事件筛选器以能够捕获状态提示事件（QEvent:：StatusTip）
...
if(trayIcon) //隐藏托盘图标，因为删除会让它一直停留到退出（在Ubuntu上）
        trayIcon->hide();
#ifdef Q_OS_MAC
...
#ifdef ENABLE_WALLET
//由于发送硬币和接收硬币，这些显示正常最小化是必要的。
//可以从托盘菜单触发，并且需要显示图形用户界面才有用。
...
//托盘图标（或停靠图标）菜单的配置
#ifndef Q_OS_MAC
//注意：在MacOS上，Dock图标的菜单已经有显示/隐藏操作。
...
{
//在初始同步、磁盘和重新索引操作时禁用MacOS应用程序nap。
#ifdef Q_OS_MAC
...
//防止孤立状态栏消息（例如，在主菜单中悬停退出，等待链同步开始->乱码文本）
...
{
QString strTitle = tr("Bitcoin"); //缺省标题
//默认为信息图标
...
void BitcoinGUI::closeEvent(QCloseEvent *event)
{
    ...
    if(clientModel && clientModel->getOptionsModel())
    {
        ...
        {
//关闭rpcconsole，以防其打开，以便为关闭窗口留出一些空间。
            rpcConsole->close();
...
//捕获状态提示事件
    ...
    {
//如果当前使用状态栏显示其他内容，请阻止从setstatustip（）添加文本。
...
encryptWalletAction->setEnabled(false); //TODO:当前不支持解密
        break;
...
//安全标志在qt gui中无效。
//bool secure=（样式&cclientuiinterface:：secure）；
    style &= ~CClientUIInterface::SECURE;
    bool ret = false;
//对于模式消息，使用阻塞连接等待用户单击按钮。

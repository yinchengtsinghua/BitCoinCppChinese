...
//将第三方事务URL添加到上下文菜单
            ...
            for (int i = 0; i < listUrls.size(); ++i)
            {
                ...
                if (!host.isEmpty())
                {
QAction *thirdPartyTxUrlAction = new QAction(host, this); //将主机用作菜单项标签
                    if (i == 0)
                        contextMenu->addSeparator();
...
//从txshashrole（qvariant/qstring）获取哈希
    uint256 hash;
...
//需要为金额重写默认的ctrl+c操作，因为默认行为只是复制displayRole文本。

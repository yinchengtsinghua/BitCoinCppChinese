...
//此文件包含源于：
//http://code.google.com/p/leveldbwin/source/browse/trunk/win32 impl_src/env_win32.h
//http://code.google.com/p/leveldbwin/source/browse/trunk/win32_impl_src/port_win32.cc
//这些文件没有任何明确的许可证头，但是
//Project（http://code.google.com/p/leveldbwin/）列出了“新的BSD许可证”
//作为许可证。
...
//虚拟void logv（可写文件*日志，const char*格式，va_list ap）；
    ...
    : filename_(fname)
{
    std::wstring path;
    ToWidePath(fname, path);
//newAppendableFile：附加到现有文件，或创建新文件
//如果不存在-这是开放的行为，
//文件附加数据以避免手动定位文件
//文件末尾的指针。
//newwritablefile:创建一个新文件，如果存在则删除-这是
//创建“始终行为”。此文件仅用于写入
//使用通用的_-write。
    _hFile = CreateFileW(path.c_str(),
                         ...
                         NULL);
//createfilew返回无效的_handle_值。如果出现错误，请在使用前始终检查isEnable（）。
}
...
if (attr == INVALID_FILE_ATTRIBUTES) { //不存在：
...
Status Win32Env::NewLogger( const std::string& fname, Logger** result )
{
    Status sRet;
    std::string path = fname;
//日志是用写语义打开的，而不是用附加语义打开的。
//（请参阅posixenv:：newlogger）

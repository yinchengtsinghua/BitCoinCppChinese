...
//fillsync—在同步模式下按随机键顺序写入n/100个值
//fill100k——在异步模式下按随机顺序写入n/1000 100k值
//deleteSeq—按顺序删除n个键
//deleteRandom—随机删除n个键
//readseq—按顺序读取n次
//read reverse—按相反顺序读取n次
//read random——随机读取n次
//自述——随机读取n个丢失的键
//readhot——从数据库的1%部分随机读取n次
//Seekrandom—n随机查找
//open—打开数据库的成本
//crc32c—4K数据的重复crc32c
//AcquireReload—加载n*1000次
//元操作：
//compact——压缩整个数据库
//stats—打印数据库统计
//sstables—打印sstable信息
//heap profile—转储堆配置文件（如果此端口支持）
...
//如果为true，则在重新打开数据库时重用现有的日志/清单文件。
...
if (ptr == NULL) exit(1); //禁用未使用的变量警告。
  ...
  void SnappyCompress(ThreadState* thread) {
    ...
    bool ok = true;
    std::string compressed;
while (ok && bytes < 1024 * 1048576) {  //压缩1G

...
///convert vector to arith_int256，via uint256 blob
...
const double R1Ldouble = 0.4887374590559308955; //R1l约等于R1ldouble*2^256
...
BOOST_AUTO_TEST_CASE( basics ) //构造器，相等，不等式
{
    BOOST_CHECK(1 == 0+1);
//构造器arith_int256（vector<char>）：
...
//检查执行“操作”是否与对每个应用“操作”的结果相同
//元素，然后将结果转换为arith_int256。
...
BOOST_AUTO_TEST_CASE( methods ) //gethex sethex size（）getlow64 getserialize大小，序列化，非序列化
{
...
for (int i = 53; i > 0; --i) //double可以将所有整数存储在0，…，2^54-1中
    {
...
BOOST_AUTO_TEST_CASE( getmaxcoverage ) //更多的测试只是为了得到100%的覆盖率
{
//~r1l给出一个基值<256>

...
//只检查sendrawtransaction的失败情况，没有网络可发送到…
...
BOOST_CHECK_THROW(AmountFromValue(ValueFromString("1e-9")), UniValue); //应该失败
BOOST_CHECK_THROW(AmountFromValue(ValueFromString("0.000000019")), UniValue); //应该失败
BOOST_CHECK_EQUAL(AmountFromValue(ValueFromString("0.00000001000000")), 1LL); //应通过，切割尾随0
BOOST_CHECK_THROW(AmountFromValue(ValueFromString("19e-9")), UniValue); //应该失败
BOOST_CHECK_EQUAL(AmountFromValue(ValueFromString("0.19e-6")), 19); //应通过，前导0存在

BOOST_CHECK_THROW(AmountFromValue(ValueFromString("92233720368.54775808")), UniValue); //溢出错误
BOOST_CHECK_THROW(AmountFromValue(ValueFromString("1e+11")), UniValue); //溢出错误
BOOST_CHECK_THROW(AmountFromValue(ValueFromString("1e11")), UniValue); //无符号溢出错误
BOOST_CHECK_THROW(AmountFromValue(ValueFromString("93e+9")), UniValue); //溢出错误
}
...
BOOST_CHECK_THROW(AmountFromValue(ParseNonRFCJSONValue(".19e-6")), std::runtime_error); //应失败，缺少前导0，因此JSON无效
...
BOOST_CHECK_THROW(r = CallRPC(std::string("setban 127.0.0.0:8334")), std::runtime_error); //不允许setban的端口号
...
BOOST_CHECK_EQUAL(banned_until.get_int64(), 9907731200); //绝对时间检查
...
BOOST_CHECK_THROW(r = CallRPC(std::string("setban test add")), std::runtime_error); //无效IP
...
feerates.emplace_back(std::make_pair(2 , 16)); //10+25%
feerates.emplace_back(std::make_pair(4 ,50)); //第50+75百分位
    feerates.emplace_back(std::make_pair(5 ,10));
feerates.emplace_back(std::make_pair(9 ,15));  //第九十百分位数
...
//与上述测试相同，但百分位重叠对中的一对被分成2个。
    total_weight = 100;
    CAmount result3[NUM_GETBLOCKSTATS_PERCENTILES] = { 0 };
    feerates.clear();

    feerates.emplace_back(std::make_pair(1, 9));
feerates.emplace_back(std::make_pair(2 , 11)); //第十百分位数
feerates.emplace_back(std::make_pair(2 , 5)); //第二十五百分位数
feerates.emplace_back(std::make_pair(4 ,50)); //第50+75百分位
    feerates.emplace_back(std::make_pair(5 ,10));
feerates.emplace_back(std::make_pair(9 ,15)); //第九十百分位数

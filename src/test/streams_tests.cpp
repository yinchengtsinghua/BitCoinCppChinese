...
//每个测试运行两次。在同一开始处序列化第二次
//点应该产生相同的结果，即使第一个测试增长了
//矢量。
...
//读取4个字节作为无符号整数。
    unsigned int c;
    reader >> c;
BOOST_CHECK_EQUAL(c, 100992003); //3,4,5,6英寸小端数基-256
...
//从缓冲区的开头读取4个字节作为带符号的int。
    VectorReader new_reader(SER_NETWORK, INIT_PROTO_VERSION, vch, 0);
    new_reader >> d;
BOOST_CHECK_EQUAL(d, 67370753); //1255,3,4英寸小端数基-256
...
BOOST_CHECK_EQUAL(serialized_int1, (uint32_t)0x7700C35A); //注意：序列化为le
    uint16_t serialized_int2;
    data >> serialized_int2;
BOOST_CHECK_EQUAL(serialized_int2, (uint16_t)0x1072); //注意：序列化为le

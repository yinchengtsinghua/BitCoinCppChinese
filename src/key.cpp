...
/*这些函数取自libsecp256k1发行版，非常难看。*/

/*
 *这将基于来自的ecprivatekey类型的der编码松散地解析格式。
 *第1节第C.4节<http://www.secg.org/sec1-v2.pdf>，附带以下注意事项：
 *
 **序列的八位字节长度必须编码为1或2个八位字节。它不是
 *如果小于256，则需要编码为一个八位字节，如DER所需。
 **序列的八位字节长度不得大于剩余长度。
 *密钥编码的长度，但不必与之匹配（即编码可能包含
 *编码序列后的垃圾）。
 **privatekey八位字节字符串在左边为零，填充到32个八位字节。
 **忽略privatekey八进制字符串编码后的任何内容，无论
 *是否有效编码。
 *
 *out32必须指向长度至少为32字节的输出缓冲区。
 **/
...
/*
 *这将从第1节的C.4节序列化为ecprivatekey类型的der编码。
 *<http://www.secg.org/sec1-v2.pdf>。可选参数和PublicKey字段是
 *包括在内。
 *
 *privkey必须指向长度至少为ckey:：private_key_size字节的输出缓冲区。
 *privkeylen最初必须设置为privkey缓冲区的大小。归还它
 *将设置为缓冲区中使用的字节数。
 *key32必须指向32字节的原始私钥。
 **/
...
//检查SIG是否具有较低的R值且小于71字节
bool SigHasLowR(const secp256k1_ecdsa_signature* sig)
{
    unsigned char compact_sig[64];
    secp256k1_ecdsa_signature_serialize_compact(secp256k1_context_sign, compact_sig, sig);

//在数据序列化中，所有值都被解释为大尾数、有符号整数。整数中的最高位表示
//它的符号性；0为正，1为负。当该值被解释为负整数时，必须将其转换
//为正值，方法是在0x00字节前加上一个值，使最高的位为0。我们可以通过确保
//我们的最高位总是0，因此我们必须检查第一个字节是否小于0x80。

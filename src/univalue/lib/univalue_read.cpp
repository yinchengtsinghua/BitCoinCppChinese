...
while (raw < end && (json_isspace(*raw)))          //跳过空白区
        raw++;
...
while (raw < end && json_isdigit(*raw)) {  //拷贝数字
            numStr += *raw;
            raw++;
        }
...
//别无所求
        ...
        案例jtok_字符串：
            if（expect（obj_name））
                univalue*top=stack.back（）；
                top->keys.push_back（标记值）；
                ClearExpect（对象名称）；
                setexpect（冒号）；
            }否则{
                单值tmpval（vstr，tokenval）；
                如果（！）Siges（））{
                    *这= TMPVAL；
                    断裂；
                }

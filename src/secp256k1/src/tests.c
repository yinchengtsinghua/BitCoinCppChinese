...
void test_rand_bits(int rand32, int bits) {
    /*（1-1/2^b）^rounds[b]<1/10^9，因此rounds是迭代次数
     *以十亿分之一的概率获得低于一次的假负概率*/
    ...
    /*对于每个maxshift+1，在一个位内使用位序列
       数字，跟踪所有观察到的结果，在uint64中每比特一个。*/
...
void test_num_negate(void) {
    secp256k1_num n1;
    secp256k1_num n2;
    /*dom_num_order_test（&n1）；/*n1=r*/
    随机数（&n1）；
    secp256k1_num_copy（&n2，&n1）；/*n2=r*/

    /*p256k1_num_sub（&n1，&n2，&n1）；/*n1=n2-n1=0*/
    检查（secp256k1_num_为_zero（&n1））；
    secp256k1_num_copy（&n1，&n2）；/*n1=r*/

    /*p256k1_num_negate（&n1）；/*n1=-r*/
    检查（！）secp256k1_num_为_zero（&n1））；
    secp256k1_num_add（&n1，&n2，&n1）；/*n1=n2+n1=0*/

    CHECK(secp256k1_num_is_zero(&n1));
    /*p256k1_num_copy（&n1，&n2）；/*n1=r*/
    secp256k1_num_negate（&n1）；/*n1=-r*/

    CHECK(secp256k1_num_is_neg(&n1) != secp256k1_num_is_neg(&n2));
    /*p256k1_num_negate（&n1）；/*n1=r*/
    检查（secp256k1_num_eq（&n1，&n2））；
}

void test_num_add_sub（void）_
    INTI；
    secp256k1_标量s；
    secp256k1_num n1；
    secp256k1_num n2；
    ...
    /*dom_num_order_test（&n2）；/*n2=r2*/
    if（secp256k1_rand_位（1））
        随机数（&n2）；
    }
    secp256k1_num_add（&n1p2，&n1，&n2）；/*n1p2=r1+r2*/

    /*p256k1_num_add（&n2p1，&n2，&n1）；/*n2p1=r2+r1*/
    secp256k1_num_sub（&n1m2，&n1，&n2）；/*n1m2=r1-r2*/

    /*p256k1_num_sub（&n2m1，&n2，&n1）；/*n2m1=r2-r1*/
    检查（secp256k1_num_eq（&n1p2，&n2p1））；
    检查（！）secp256k1_num_eq（&n1p2，&n1m2））；
    secp256k1_num_negate（&n2m1）；/*n2m1=-r2+r1*/

    CHECK(secp256k1_num_eq(&n2m1, &n1m2));
    CHECK(!secp256k1_num_eq(&n2m1, &n1));
    /*p256k1_num_add（&n2m1，&n2m1，&n2）；/*n2m1=-r2+r1+r2=r1*/
    检查（secp256k1_num_eq（&n2m1，&n1））；
    检查（！）secp256k1_num_eq（&n2p1，&n1））；
    ...
    for (i = 0; i < 250; ++i) {
        /*p256k1_num_add（&n1，&n1，&n1，&n1）；/*n1*=2*/
...
void test_num_mod(void) {
    ...
    /*检查将数字增加到2^256之后是否不会破坏此值*/
    random_scalar_order_test(&s);
    secp256k1_scalar_get_num(&n, &s);
    /*乘以2^8，这将用很高的概率测试这个案例*/
    for (i = 0; i < 8; ++i) {
...
void test_num_jacobi(void) {
    ...
    /*ck（secp256k1_num_jacobi（&n，&order）==1）；/*sage确认这是1*/
}

void运行_num_smalltests（void）
    INTI；
    对于（i=0；i<100*计数；i++）
        test_num_negate（）；
        test_num_add_sub（）；
...
void scalar_test(void) {
    ...
    /*将“s1”设置为随机标量，值为“s1num”。*/
    random_scalar_order_test(&s1);

    /*将“s2”设置为随机标量，值为“snum2”，字节数组表示形式为“c”。*/
    ...
    {
        int i;
        /*测试从标量中提取4位的组，递归n（i）=16*n（i-1）+p（i）重建它。*/
        secp256k1_scalar n;
    ...
    {
        /*测试从一个标量中获取随机大小的位组，并递归n（i）=b*n（i-1）+p（i）重建它。*/
        secp256k1_scalar n;
        int i = 0;
        secp256k1_scalar_set_int(&n, 0);
        while (i < 256) {
...

    {
        ...
        /*检查与半阶的比较是否等于否定后对高标量的测试。*/
        CHECK(secp256k1_scalar_is_high(&neg) == (secp256k1_num_cmp(&negnum, &half_order) > 0));
        /*除非值已经为零，否则求反应该更改high属性。*/
        CHECK((secp256k1_scalar_is_high(&s) == secp256k1_scalar_is_high(&neg)) == secp256k1_scalar_is_zero(&s));
        secp256k1_scalar_get_num(&negnum2, &neg);
        /*求反标量应该等于数字上的（order-n）mod order。*/
    ...
    {
        /*测试secp256k1_scalar_mul_shift_var。*/
        secp256k1_scalar r;
        secp256k1_num one;
        secp256k1_num rnum;
        secp256k1_num rnum2;
...
void run_scalar_tests(void) {
    ...
    {
        /*静态测试向量。
         *根据比较决定，从~10^12个随机向量中减去这些向量。
         *以及32位和64位实现上的边缘大小写覆盖率。
         *使用SAGE 5.9生成响应。
         **/
...
void test_ge(void) {
    ...
    int runs = 4;
#endif
    /*点：（无穷大，p1，p1，-p1，-p1，p2，p2，-p2，p3，p3，-p3，-p3，p4，p4，-p4，-p4）。
     *每对相同点中的第二个点使用雅可比形式的随机Z坐标。
     *所有震级都是随机的。
     *所有17*17点组合使用所有适用的方法相互添加。
     *
     *在编译自同态代码时，还会添加p5=lambda*p1和p6=lambda^2*p1。
     **/
    ...
    /*生成随机ZF，ZFI2=1/ZF^2，ZFI3=1/ZF^3*/
    do {
    ...
    for (i1 = 0; i1 < 1 + 4 * runs; i1++) {
        ...
        for (i2 = 0; i2 < 1 + 4 * runs; i2++) {
            ...
            /*测试gej+ge（var，附加z系数）。*/
            {
                /*p256k1_-ge-ge2_-zfi=ge[i2]；/*第二项x和y重新调整z=1/zf*/
                secp256k1_fe_mul（&ge2_zfi.x，&ge2_zfi.x，&zfi2）；
                secp256k1_fe_mul（&ge2_zfi.y，&ge2_zfi.y，&zfi3）；
                随机_字段_元素_大小（&ge2 _zfi.x）；
                随机_字段_元素_大小（&ge2_zfi.y）；
            ...
            if (i2 != 0) {
                /*secp256k1_gej_add_ge不支持其第二个参数无穷大。*/
...
void test_add_neg_y_diff_x(void) {
    /*这个测试的重点是检查我们是否可以添加两个点
     *其y坐标彼此为负，但其x
     *坐标不同。如果x坐标相同，这些
     *分数为负数，其和为
     *无穷大。这很酷，因为它“掩盖”了任何退化。
     *在会导致xy坐标的加法算法中
     *和错误（因为无穷大没有xy坐标）。
     *但是，如果x坐标不同，无穷大是
     *回答错误，会暴露出这种退化。这就是
     *https://github.com/bitcoin-core/secp256k1/issues/257的根目录
     *此测试是针对哪个的回归测试。
     *
     *这些点在SAGE中生成为
     *secp256k1参数
     ...
     *c=椭圆曲线（[f（0），f（7）]）
     *G=C.提升装置x（0x79BE667EF9DCBBAC55A06295CE870B07029BFCDB2DCE28D959F2815B16F81798）
     *n=finitefield（g.order（））
     *
     *自同态值（lambda在n中为1 ^ 1/3，beta在f中为1 ^ 1/3）
     *x=聚原（n）
     *lam=（1-x^3）.roots（）[1][0]
     *
     *随机“坏对”
...
void test_group_decompress(const secp256k1_fe* x) {
    ...
    secp256k1_fe fex = *x;
    secp256k1_fe fez;
    /*set_xquad_var，set_xo_var（…，0），set_xo_var（…，1）的结果。*/
    ...
    if (res_quad) {
        ...
        /*检查secp256k1_gej_has_quad_y_var。*/
...
void run_ecmult_chain(void) {
    ...
    for (i = 0; i < 200*count; i++) {
        /*在每次迭代中，计算x=xn*x+gn*g；*/
        secp256k1_ecmult(&ctx->ecmult_ctx, &x, &x, &xn, &gn);
        /*同时计算ae和ge:a和g的实际累积因子*/
        /*如果x是（ae*a+ge*g），xn*x+gn*g会导致（xn*ae*a+（xn*ge+gn）*g）*/
...
void test_point_times_order(const secp256k1_gej *point) {
    ...
    /*p256k1_ecmult（&ctx->ecmult_ctx，&res1，point，&x，&x）；/*计算res1=x*point+x*g；*/
    secp256k1_ecmult（&ctx->ecmult_ctx，&res2，point，&nx，&nx）；/*计算res2=（order-x）*point+（order-x）*g；*/
...
void test_wnaf(const secp256k1_scalar *number, int w) {
    ...
    for (i = bits-1; i >= 0; i--) {
        ...
        if (v) {
            /*ck（zeroes==-1 zeroes>=w-1）；/*检查非零元素之间的距离是否至少为w-1*/
            零点＝0；
            检查（（v&1）==1）；/*检查非零元素为奇数*/
    ...
    检查（secp256k1_scalar_eq（&x，number））；/*检查wnaf是否代表数字*/
...
void test_ecmult_constants(void) {
    /*测试ecmult_gen（）的[0..36]和[order-36..0]。*/
    secp256k1_scalar x;
    secp256k1_gej r;
    secp256k1_ge ng;
    int i;
...
void test_ecmult_gen_blind(void) {
    /*测试ecmult_gen（）致盲，并确认致盲更改、仿射点匹配以及Z不匹配。*/
...
void ec_pubkey_parse_pointtest(const unsigned char *input, int xvalid, int yvalid) {
    ...
    for (pubkeyclen = 3; pubkeyclen <= 65; pubkeyclen++) {
        ...
        for (i = 0; i < 256; i++) {
            ...
            /*这一点有什么标志？*/
            ysign = (input[63] & 1) + 2;
            /*对于当前类型（i），我们希望解析可以工作吗？处理所有压缩/未压缩/混合。*/
            xpass = xvalid && (pubkeyclen == 33) && ((i & 254) == 2);
            /*我们是否期望一个未压缩的解析和重新序列化得到一个匹配的y？*/
            ...
            if (xpass || ypass) {
                ...
                if (ypass) {
                    /*这个测试并不总是完成的，因为我们用替代符号解码，所以Y不匹配。*/
...
#define SECP256K1_EC_PARSE_TEST_NINVALID (7)
    const unsigned char invalid[SECP256K1_EC_PARSE_TEST_NINVALID][64] = {
        {
            /*x是-8的第三个根，y是-1*（x^3+7）；也在y^2=x^3+9的曲线上。*/
        ...
        {
            /*x是-1，y是sqrt阶梯的结果；也在y^2=x^3-5的曲线上。*/
            0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
            0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe, 0xff, 0xff, 0xfc, 0x2e,
        ...
        {
            /*x是零，y是sqrt阶梯的结果；也在y^2=x^3-7的曲线上。*/
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    ecount = 0;
    VG_UNDEF(&pubkey, sizeof(pubkey));
    /*从0x04开始的有效输入声明了33个字节，失败，归零输出，没有非法的arg错误。*/
    CHECK(secp256k1_ec_pubkey_parse(ctx, &pubkey, pubkeyc, 33) == 0);
    VG_CHECK(&pubkey, sizeof(pubkey));
    CHECK(ecount == 0);
    CHECK(secp256k1_pubkey_load(ctx, &ge, &pubkey) == 0);
    CHECK(ecount == 1);
    /*空pubkey，非法arg错误。pubkey在这个步骤之前不会被重写，因为它在解析器中为空。*/
    CHECK(secp256k1_ec_pubkey_parse(ctx, NULL, pubkeyc, 65) == 0);
    CHECK(ecount == 2);
    /*输入字符串为空。非法的arg和zeroize输出。*/
    memset(&pubkey, 0xfe, sizeof(pubkey));
    ecount = 0;
    VG_UNDEF(&pubkey, sizeof(pubkey));
    ...
    CHECK(secp256k1_pubkey_load(ctx, &ge, &pubkey) == 0);
    CHECK(ecount == 2);
    /*以0x04开头的输入声明64个字节，失败，将输出归零，没有非法的arg错误。*/
    memset(&pubkey, 0xfe, sizeof(pubkey));
    ecount = 0;
    VG_UNDEF(&pubkey, sizeof(pubkey));
...
/*伪nonce生成函数，只使用预计算的nonce，如果不被接受，则会失败。仅用于测试。*/
...
static int nonce_function_test_retry(unsigned char *nonce32, const unsigned char *msg32, const unsigned char *key32, const unsigned char *algo16, void *data, unsigned int counter) {
   /*为前几个计数器值生成不可接受的nonce的伪nonce生成器。*/
   if (counter < 3) {
   ...
   /*6979的重试率是可以忽略的，特别是我们只在确定性测试中调用它。*/
   /*如果有人对secp256k1重试的案例做了罚款，我们想知道。*/
   if (counter > 5) {
       return 0;
   }
...
void test_ecdsa_end_to_end(void) {
    ...
    /*测试Lower-S Form，malleate，验证并失败，再次测试，malleate再次*/
...
static void random_ber_signature(unsigned char *sig, size_t *len, int* certainly_der, int* certainly_not_der) {
    ...
    *certainly_not_der = 0;
    ...
    for (n = 0; n < 2; n++) {
        /*我们生成两类数字：nlow==1“低”个（最多32个字节），nlow==0“高”个（32个字节，设置129个高位，或大于32个字节）*/
        nlow[n] = der ? 1 : (secp256k1_rand_bits(3) != 0);
        /*以字节为单位的数字长度（第一个字节将始终为非零）*/
...
void test_ecdsa_edge_cases(void) {
    ...
    /*S为零的签名。*/
    {
        ...
        CHECK(secp256k1_ec_pubkey_create(ctx, &pubkey, NULL) == 0);
        CHECK(ecount == 7);
        /*PubKeyLoad通过argCheck失败有点奇怪，但有意义，因为PubKeys是不透明的数据类型。*/
